...
```{r}
set.seed(123)
model_benchmark <- train(class ~ ., data = data_benchmark, method = "glm", family = "binomial", trControl = trainControl(method = "LOOCV"))
...
data_7 <- data %>%
  select(sig_7, class) %>% 
  train(class ~ ., data = ., method = "glm", family = "binomial", trControl = trainControl(method = "LOOCV"))
