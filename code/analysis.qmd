...
Even though we have reduced the number of variables to `r nrow(sig_001)`, the algorithm still does not seem to converge. In the li
...
```{r}
logit_7 <- data_pca %>% 
  select(pc1, pc2, pc3, pc4, pc5, pc6, pc7, class) %>%
  train(
    class ~ .,
    data = .,
    method = "glm",
